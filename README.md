...
{ 
  ...
  "comments": "Добрый день! Подскажите, когда началась проблема? ||| Проблема началась после обновления ||| ..." 
} 
</pre>

Важно!

Комментарии необходимо выгружать следующим образом из БД, агрегируя их разделителем:

<pre>
STRING_AGG(comments.comments, ' ||| ' order by comments.date_comments)
</pre>

Данный разделитель (|||) используется для подготовки блока комментариев и их очистки.
