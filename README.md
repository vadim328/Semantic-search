...
🚀 Основные возможности
    ...
    🎯 Фильтрация по:
        ...
        диапазону дат
    🧠 Как это работает
        1) Входной запрос преобразуется в эмбеддинг с помощью BERT-модели
...
│   └── api/
│       └── routes/
│           └── search_routes.py        # Маршрут для поиска
│           └── summarization_routes.py # Маршрут для суммаризации
│           └── validate_params.py      # Проверка обязательных параметров
│   └── db/
│       └── relational_db/
...
│       └── text_preparation.py         # Классы для обработки текста
│
...
- service - блок с настройками сервиса
  ...
  - updater
    - max_concurrent - Количество одновременных потоков для обработки полученных строк из реляционной БД. Рекомендуемое значение - количество воркеров для сервиса моделей
